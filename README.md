...
The `USART_setting()` function initializes a selected USART (1, 2, or 6) by configuring TX/RX pins, enabling clocks, setting the baud rate, and enabling transmitter, receiver, and RX interrupt for serial communication.
...
The `USART_write()` function sends a sequence of bytes through the specified USART by waiting for the transmit buffer to be empty (TXE flag) before writing each byte to the data register. After all data is sent, it waits for the transmission complete (TC) flag to ensure all bits are fully transmitted, then clears the TC flag.
