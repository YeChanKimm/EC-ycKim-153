...
// 4. Configure Timer Output mode as PWM
	...
	if(chN == 1){
		...
		TIMx->CCMR1 |= TIM_CCMR1_OC2M_1 | TIM_CCMR1_OC2M_2;                     					// Output 1 preload enable (make CCR2 value changable)	
		TIMx->CCMR1	|= TIM_CCMR1_OC2PE;															// Output Compare Register for channel 2 (default duty ratio = 50%)		
		TIMx->CCR2  = ccVal;                       				// select output polarity: active high	
...
// 1. Configure prescaler PSC
	uint32_t value = ((TIMx->ARR+1)*duty);    								// (ARR+1)*dutyRatio

	if(duty <= 0.f) value = 0;
	else if(duty >= 1.f) value = TIMx->ARR + 1;
